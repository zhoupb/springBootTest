...
public class RedisUtil {
    ...
    public Boolean scan(String key, String value) {
        try {
            return redisTemplate.opsForZSet().scan(key, ScanOptions.scanOptions().match(value).build()).hasNext();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
     ...
     *
     * @param key   设置缓存key
     * @param value 设置缓存value
     * @param time  失效时间
     * @return
     */
    public Long pfAdd(String key, Object value, long time) {
        Long count = redisTemplate.opsForHyperLogLog().add(key, value == null ? "1" : value);
        if (time > 0) {
     ...
     * 将数据存储HyperLogLog中
     *
     * @param key  设置缓存key,默认值为"1"
     * @param time 失效时间
     * @return
     */
    public Long pfAdd(String key, long time) {
        return this.pfAdd(key, null, time);
    }
